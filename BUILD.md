...
Édite `package.json`, change la ligne `"version": "16.0.0"` selon ce que tu veux.

**Important** : pense à mettre la même version dans `installer/mcl-sit-installer.iss` à la ligne `#define AppVersion "16.0.0"`. Sinon le setup et l'app afficheront des versions différentes.
...
4. Quand c'est fini, le fichier `MCL-SIT-Setup-16.0.0.exe` est dans `installer\output\`

### Étape 4 : tester l'installeur

1. Récupérer `installer\output\MCL-SIT-Setup-16.0.0.exe`
...
└── installer/
    ...
    └── output/
        └── MCL-SIT-Setup-16.0.0.exe       ← LIVRABLE FINAL
```
